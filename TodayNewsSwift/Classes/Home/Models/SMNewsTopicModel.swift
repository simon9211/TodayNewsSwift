//
//  SMNewsTopicModel.swift
//  TodayNewsSwift
//
//  Created by xiwang wang on 2017/6/3.
//  Copyright © 2017年 xiwang wang. All rights reserved.
//

import UIKit

class SMNewsTopicModel: NSObject {
    // 文字的高度
    ...
    var video_detail_info: SMVideoDetailInfo?
    ...
    // 点击 『x』 按钮，弹出框内容
    var filter_words = [SMFilterWord]()
    
    var image_list = [SMImageList]()
    var middle_image: SMMiddleImage?
    var large_image_list = [SMLargeImageList]()
    ...
    init(dict: [String: AnyObject]) {
        ...
        if let filterWords = dict["filter_words"] as? [AnyObject] {
            for item in filterWords {
                let filterWord = SMFilterWord(dict: item as! [String: AnyObject])
        ...
        if let mediaDict = dict["media_info"] {
            media_info = SMMediaInfo(dict: mediaDict as! [String: AnyObject])
        }
        
        if let videoDetailInfo = dict["video_detail_info"] {
            video_detail_info = SMVideoDetailInfo(dict: videoDetailInfo as! [String : AnyObject])
        }
        
        if let middleImage = dict["middle_image"] {
            middle_image = SMMiddleImage(dict: middleImage as! [String: AnyObject])
        }
        ...
        if imageLists == nil || imageLists?.count == 0 {
            ...
            if middle_image?.height != nil {
                ...
                if video_detail_info?.video_id != nil || largeImageLists!.count > 0 {
                    ...
                    imageH = 170
                    titleW = SCREENW - 30
                    titleH = title!.boundingRect(with: CGSize(width: titleW, height: CGFloat(MAXFLOAT)), options: .usesLineFragmentOrigin, attributes: [NSFontAttributeName: UIFont.systemFont(ofSize: 17)], context: nil).size.height
                    ...
                    if largeImageLists!.count > 0 {
                        for index in largeImageLists! {
                            let largeImage = SMLargeImageList(dict: index as! [String : AnyObject])
            ...
            for item in imageLists! {
                let imageList = SMImageList(dict: item as! [String: AnyObject])
...
class SMMiddleImage: NSObject {
    ...
    init(dict: [String: AnyObject]) {
        ...
        width = dict["width"] as? Int
        if let urlString = dict["url"] as? NSString {
            if urlString.hasSuffix(".webp") {
                let range = urlString.range(of: ".webp")
                url = urlString.substring(to: range.location)
            } else {
...
class SMLargeImageList: NSObject {
...
class SMVideoDetailInfo: NSObject {
    ...
    var detail_video_large_image: SMDetailVideoLargeImage?
    ...
    init(dict: [String: AnyObject]) {
        ...
        detail_video_large_image = SMDetailVideoLargeImage(dict: dict["detail_video_large_image"] as! [String: AnyObject])
    }
}

class SMDetailVideoLargeImage: NSObject {
